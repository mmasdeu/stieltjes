...
@[ext]
structure Selection (I : NonemptyInterval ℝ) where
  T : Partition I
  map : ℝ → ℝ → ℝ
  loc : ∀ (t₁ t₂ : ℝ), t₁ ∈ T.carrier → t₂ ∈ T.carrier → t₁ ≤ t₂ → map t₁ t₂ ∈ Set.Icc t₁ t₂
...
instance (P : Partition I) : Fintype {⟨t₁, t₂⟩ :P.carrier × P.carrier | P.Adjacent t₁ t₂} := Fintype.ofFinite _

def Intervals (P : Partition I) : Finset (P.carrier × P.carrier) := {⟨t₁, t₂⟩ : P.carrier × P.carrier | P.Adjacent t₁ t₂}.toFinset

def Darboux (f : ℝ → ℝ) (α : ℝ → ℝ) (S : Selection I) :=
  ∑ a in (S.T.Intervals), f (S.map a.1 a.2) * (α (a.2) - α (a.1))
